var Dashboard = function() {
    ...
    return {
        ...
        initAmChart3: function() {
            ...
            $.ajax({
                  type: "POST",
                  url: "marketing/get_reserve_fees",
                  dataType: 'json',
                  success: function(data)
                       {
                        ...
                        $.each(data, function (index, value){
                            // console.log(data[index].months);
                            var arr_val = {
                                'year' : month_name[data[index].months - 1]  + '(' + data[index].year + ')',
                                'income' : data[index].reserve_fees
                            };
                            arr_reserve.push(arr_val);
                        });
                        $.each(arr_reserve, function (index, value){
                            console.log(arr_reserve[index].month);
                        });
...
if (typeof(AmCharts) === 'undefined' || $('#dashboard_amchart_tcp').size() === 0) {
                return;
            }
                  ...
                  success: function(data)
                       {
                        ...
                        $.each(data, function (index, value){
                            ...
                            var arr_val = {
                                'year' : month_name[data[index].months - 1]  + '(' + data[index].year + ')',
                                'income' : data[index].total_tcp
                            };
                            arr_tcp.push(arr_val);
                        });
                        $.each(arr_tcp, function (index, value){
                            console.log(arr_tcp[index].month);
                        });


                        var chart = AmCharts.makeChart("dashboard_amchart_tcp", {
                                "type": "serial",
                                "addClassNames": true,
                                "theme": "light",
                                "path": "../assets/global/plugins/amcharts/ammap/images/",
