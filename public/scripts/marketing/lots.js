                ...
                if (lot_id != null || lot_id != 0) {
                  $.ajax({
                      ...
                      success: function(data){
                      	// alert(lot_id);
                        $("#lot_id").val(lot_id);
                      	$("#txt_project_name").html(data[0].requested_by_id);
                        ...
                        $("#lot_vat").val(data[0].lot_vat);
                        console.log(data[0].lot_vat);
                        $("#total_price").val(jFormatNumber((parseFloat(data[0].lot_area * data[0].price_per_sqr_meter) + parseFloat(data[0].house_price) + parseFloat(data[0].lot_vat))));
        ...
        $.ajax({ 
              ...
              success: function(data){
                          ...
                          for (var i = 0; i < data.length; i++) {
                              ...
                              var price = 0;
                              // price = parseFloat((jFormatNumberRet(parseFloat(data[i].lot_area)) * jFormatNumberRet(parseFloat(data[i].price_per_sqr_meter))) + (jFormatNumberRet(parseFloat(data[i].house_price)) + jFormatNumberRet(parseFloat(data[i].lot_vat))));
                              price = parseFloat((jFormatNumberRet(data[i].lot_area) * jFormatNumberRet(data[i].price_per_sqr_meter))) + parseFloat(jFormatNumberRet(data[i].house_price) + jFormatNumberRet(data[i].lot_vat));
                              
                              lots_table.row.add( [
                                            data[i].lot_id,
                                            data[i].lot_description,
                                            data[i].lot_area + " Sq m",
                                            (data[i].price_per_sqr_meter),
                                            (data[i].house_price),
                                            (data[i].lot_vat),
                                            (price),
                                            av,
        ...
        $.ajax({ 
              ...
              success: function(data){
                          ...
                          for (var i = 0; i < data.length; i++) {
                              ...
                              var price = 0;
                              price = parseFloat((jFormatNumberRet(data[i].lot_area) * jFormatNumberRet(data[i].price_per_sqr_meter))) + parseFloat(jFormatNumberRet(data[i].house_price) + jFormatNumberRet(data[i].lot_vat));
                              // price = parseFloat((jFormatNumberRet((data[i].lot_area)) * jFormatNumberRet((data[i].price_per_sqr_meter))) + (jFormatNumberRet((data[i].house_price)) + jFormatNumberRet((data[i].lot_vat))));
