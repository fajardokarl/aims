$(document).ready(function(){
	...
	$('#generate').on('click', function() {
		...
		if($('#sortby').val() == 1){
			...
			$.ajax({
	            type: "POST",
	            url:  baseurl + "collection/get_daily_collection_per_project",
	            ...
	            success: function(result){
	                ...
	                if(data.length==0){
	                	...
	                	for(x=0;x<data.length;x++){
	                		...
	                		content += '<td align="right">' + numberWithCommas(data[x].amount) + '</td>';
	                		if (!data[x].principal){
	                			content += '<td align="right">0.00</td>';
	                		} else {
	                			content += '<td align="right">' + numberWithCommas(data[x].principal) + '</td>';
	                		}
	                		if (!data[x].interest){
	                			content += '<td align="right">0.00</td>';
	                		} else {
	                			content += '<td align="right">' + numberWithCommas(data[x].interest) + '</td>';
	                		}
	                		if (!data[x].surcharge){
	                			content += '<td align="right">0.00</td>';
	                		} else {
	                			content += '<td align="right">' + numberWithCommas(data[x].surcharge) + '</td>';
	                		}
	                		if (!data[x].ips){
	                			content += '<td align="right">0.00</td>';
	                		} else {
	                			content += '<td align="right">' + numberWithCommas(data[x].ips) + '</td>';
	                		}
	                		if (!data[x].sundry){
	                			content += '<td align="right">0.00</td>';
	                		} else {
	                			content += '<td align="right">' + numberWithCommas(data[x].sundry) + '</td>';
	                		}
	                	}
		...
		if($('#sortby').val() == 2) {
			...
			$.ajax({
	            type: "POST",
	            url:  baseurl + "collection/get_daily_collection_per_vat",
	            data: {'vatid':vatid},
	            success: function(result){
	...
	$('#print').click(function () {
        ...
        $.ajax({
            type: "POST",
            url:  baseurl + "collection/print_daily_collection",
            data: {'data':data,'title':title,'date':date},
            success: function(data){
                var url = baseurl + "reports/DailyCollection.pdf";
    ...
    $('#excel').click(function () {
        ...
        $.ajax({
            type: "POST",
            url:  baseurl + "collection/get_daily_collection_report",
            data: {'data':data,'title':title,'date':date},
            success: function(data){
