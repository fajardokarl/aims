...
$(document).ready(function(){
        ...
        $.ajax({
            ...
            success: function(result){
                ...
                if (data['contract'].length != 0){
                    ...
                    var content = '';
                    var tcp = Number(data['contract'][0].total_contract_price);
                    for (var i = 0; i < data['payment'].length; i++) {
                        content += '<tr>';
                        content += '<td class="amortid" style="display: none;">' + data['payment'][i].amortization_id + '</td>';
                        content += '<td class="paymentid" style="display: none;">' + data['payment'][i].payment_id + '</td>';
                        content += '<td class="paymenttypeid" style="display: none;">' + data['payment'][i].pay_reference + '</td>';
                        content += '<td>' + data['payment'][i].pay_date + '</td>';
                        content += '<td>' + data['payment'][i].payment_name + '</td>';
                        content += '<td align="right">' + numberWithCommas(Number(data['payment'][i].amount).toFixed(2)) + '</td>';
                        content += '<td align="right" class="principal">' + numberWithCommas(Number(data['payment'][i].principal).toFixed(2)) + '</td>';
                        content += '<td align="right" class="interest">' + numberWithCommas(Number(data['payment'][i].interest).toFixed(2)) + '</td>';
                        content += '<td align="right" class="surcharge">' + numberWithCommas(Number(data['payment'][i].surcharge).toFixed(2)) + '</td>';
                        content += '<td align="right" class="sundry">' + numberWithCommas(Number(data['payment'][i].sundry).toFixed(2)) + '</td>';
                        content += '<td align="right" class="ips_interest">' + numberWithCommas(Number(data['payment'][i].ips_interest).toFixed(2)) + '</td>';
                        content += '<td align="right" class="ips_accrued">' + numberWithCommas(Number(data['payment'][i].ips_accrued).toFixed(2)) + '</td>';
                        var remainingbalance = getRemainingBalance(contractid,data['payment'][i].amortization_id);
                        content += '<td align="right">' + numberWithCommas(remainingbalance.toFixed(2)) + '</td>';
                        content += '<td><button class="btn btn-circle green cancel-payment">Cancel Payment</button></td>';
                        content += '</tr>';
                    }
                    $('#tbody_recompute').html(content);
                    var removeDataTable = $('#tblrecompute').dataTable();
                    removeDataTable.fnDestroy();
                    $('#tblrecompute').DataTable();
                    $('#details2').show();
    ...
    $(document).on('click', ".cancel-payment", function() {
        var amortizationid = $(this).closest("tr").find(".amortid").text();
        var paymentid = $(this).closest("tr").find(".paymentid").text();
        var paymenttypeid = $(this).closest("tr").find(".paymenttypeid").text();
        var principal = $(this).closest("tr").find(".principal").text().replace(/,/g, '');
        var interest = $(this).closest("tr").find(".interest").text().replace(/,/g, '');
        var surcharge = $(this).closest("tr").find(".surcharge").text().replace(/,/g, '');
        var sundry = $(this).closest("tr").find(".sundry").text().replace(/,/g, '');
        var ips_interest = $(this).closest("tr").find(".ips_interest").text().replace(/,/g, '');
        var ips_accrued = $(this).closest("tr").find(".ips_accrued").text().replace(/,/g, '');

        var data = new FormData();
        data.append('amortizationid',amortizationid);
        ...
        data.append('ips_interest',ips_interest);
        data.append('ips_accrued',ips_accrued);
        ...
        $('#yes').click(function () {
            $.ajax({
                async: false,
                type: "POST",
                url:  baseurl + "collection/cancelPayment",
                data: data,
                cache: false,
                processData:false,
                contentType:false,
    ...
    //     console.log('wiw');
        ...
        // console.log('amortid '+amortizationid);
        // console.log('paymentid '+paymenttypeid);
        // console.log('paymenttypeid '+paymenttypeid);
    // });

    // $(document).on("click","#tblrecompute tr",function() {
    //     $("<style type='text/css'> .highlight{ background:#33F0FF;} </style>").appendTo("head");
    //     $("#tblrecompute tr").each(function () {
    //         if ( $(this).hasClass('highlight') ) {
    //             $(this).removeClass('highlight');
    //         }
    //     });
    ...
    //     var table = document.getElementById("tblrecompute");
    //     var row = $(this).parent().children().index($(this));      
    //     row = Number(row);
    //     row = +row + 1;
    //     console.log(row);
    //     var amortizationid = table.rows[row].cells[0].innerHTML;

    //     var count=0;
    //     var numrows = $("#tblrecompute").dataTable().fnGetNodes();
...
function getRemainingBalance(contractid, amortizationid) {
    ...
    $.ajax({
        async: false,
        type: "POST",
        url:  baseurl + "collection/getRemainingBalance",
        data: data,
        cache: false,
        processData:false,
        contentType:false,
        success: function(result){
            var data = jQuery.parseJSON(result);
            var tcp = Number(data['contract'][0].total_contract_price);
            var discount = Number(data['contract'][0].downpayment_discount);
            var discounted_tcp = tcp - discount;
            var total_paid = Number(data['amortization'][0].total_paid);
            remainingbalance = discounted_tcp - total_paid;
        },
        error: function (errorThrown){
