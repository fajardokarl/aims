...
$(document).ready(function(){
    ...
    $('#tblnvrpayments').on('click', 'input[type="button"]', function(e){
    ...
    $('#add').click(function() {
        ...
        if($('#bookcode1').val()=="" || amount<1 || $('#accountcode').val()==""){
            ...
            $('#tblnvrpayments tr:last').after('<tr><td>'+account_code+'</td><td>'+account_description+'</td><td>'+book_code+'</td><td>'+book_prefix+'</td><td>'+debit+'</td><td>'+credit+'</td><td><input type="button" value="Delete" onclick="deleteRow(this)"></td></tr>');
        }

    });    

    $('#save').click(function() {
        var table = document.getElementById( "tblnvrpayments" );
        ...
        if (total_amount >= total_debit_and_credit){
            if($('#paymenttype').val()==1){
                if($('#paymentdate').val()=='' || $('#cash').val()==0){
                    ...
                    $.ajax({
                        type: "POST",
                        url:  baseurl + "collection/pay_nvr",
                        data: {'paymenttype':paymenttype,'paymentdate':paymentdate,'cashamount':cashamount,'customername':customername,
                        'subcode':subcode,'subtype':subtype,'data':data,'cashier_id':user_id,'book_code_top':book_code_top,'book_prefix_top':book_prefix_top},
                        success: function(result){
                            toastr.success('Successfully Saved!', 'Operation Done');
                            var url = baseurl + "reports/NonVatReceipt.pdf";
            ...
            if($('#paymenttype').val()==3){
                if($('#paymentdate').val()=='' || $('#cash').val()==0 || $('#checkamount').val()==0 || $('#checknumber').val()=='' || $('#checkdate').val()=='' || $('#bank2').val()==0){
                    ...
                    $.ajax({
                        ...
                        'book_prefix_top':book_prefix_top},
                        success: function(result){
                            toastr.success('Successfully Saved!', 'Operation Done');
                            var url = baseurl + "reports/NonVatReceipt.pdf";
                            var win = window.open(url, '_blank');
