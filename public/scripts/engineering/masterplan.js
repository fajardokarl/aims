$(document).ready(function(){
	...
	$('#opt_project').change(function(){
		...
		$.ajax({
			type: "POST",
            url : baseurl + "engineering/get_all_phase",
            dataType : "json",
	...
	$("#check_add_all").click( function(){
        if($(this).is(':checked') ){
			...
			$.ajax({
				type: "POST",
	            url : baseurl + "engineering/get_phases",
	            dataType : "json",
	            success : function(data){
	            	$.each(data, function(i, value){
	                    opt_content += "<option value='" + data[i].phase_id + "'>" + data[i].phase_name + "</option>"
	...
	$('#save_all_lots').click(function(){
		...
		if (lot_row_num > 0) {
			...
			$.ajax({
				type: "POST",
	            url : baseurl + "engineering/save_lots",
	            data: {'lot_arr' : lot_arr, 'proj_name' : $('#new_proj_name').val(), 'proj_desc' : $('#new_proj_desc').val(), 'new_proj' : new_proj},
	            success : function(data){
		            	// toastr.options.timeOut = 500;
