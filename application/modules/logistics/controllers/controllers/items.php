...
class Items extends CI_Controller
{
	public function __construct(){
		parent::__construct();
		$this->load->model('items_model', 'request');
		$this->load->model('route_model');
		$this->load->helper(array('form', 'url'));
		// $this->data['navigation'] = 'logistics/navigation';
		$this->data['customjs'] = 'items_js';
	}

	public function index(){
		$this->data['content'] = 'items_view';
		$this->data['page_title'] = 'Item Management';
	...
	public function get_item_specs()
	{
		$this->db->trans_start();
		$this->db->select("category, brand, color, size");
		$this->db->from("specs");
		$this->db->where("specs_id", $_POST['data']['specs_id']);
		$this->db->where("item_id", $_POST['data']['item_id']);
		$query = $this->db->get();
		$this->db->trans_complete();
		echo json_encode($query->result_array());
	}

	public function edit_item_specs()
	{
		$item_specs = $_POST['item_specs'];
		echo json_encode($this->request->edit_item_specs($item_specs));
	}

	public function ajax_delete_spec()
	{
		echo json_encode($this->request->delete_spec($_POST['spec_id']));
	}
