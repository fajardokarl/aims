...
class Users_model extends CI_Model
{
    ...
    function get_users()
    {
        $this->db->select('*');
        $this->db->from('user');
        $this->db->join('person', 'user.person_id = person.person_id', 'left');
        $this->db->join('employee', 'employee.employee_id = person.person_id', 'left');
        ...
        $this->db->where('user.status_id >', 0);
        $query = $this->db->get();
