...
class admin_issuance extends CI_Controller
{
    public function __construct() {
        parent::__construct();
        $this->load->model('admin_issuance_model', 'as');
        $this->load->helper(array('form', 'url'));
        $this->data['navigation'] = 'warehouse/navigation';
        $this->data['customjs'] = 'warehouse/admin_issuancejs';
        $this->data['verifies'] = $this->as->get_verifies();
    }

    public function index() {
        $this->data['content'] = 'issuance';
        $this->data['page_title'] = 'Materials Issuance Slip';
        // $this->data['RR_details'] = $this->as->retrieve_receiving_report_details();
        $this->data['MR_details'] = $this->as->getMaterialsRequisition();
        $this->data['warehouse'] = $this->as->get_warehouse_model();
        $this->data['project'] = $this->as->get_allproject_model();
        $this->data['issuance'] = $this->as->get_allissuance_model();
    ...
    public function process_issuance() {
        ?>
        <pre>
        ...
        // print_r($_POST);
        // echo '</pre>';
        if($this->input->post('issuance_button') == 1) {
            $status = "CONFIRMED";
        } else {
            $status = "CANCELLED";
        }
        ...
        $info1 = [
            ...
            'issuance_project' => $_POST['opt_iss_project'],
            'warehouse_id' => $_POST['opt_iss_warehouse']
        ];
        $this->as->confirm_issuance($info1, $issuance_id);
        ...
        while($i < $_POST['inc']) {
            $isd_id = explode('-isd_id', $names1[$i]);
            $item_id = explode('-item_id', $names2[$i]);
            $qty_received = $_POST[$isd_id[0] . '-isd_item_qty_received'];

            $info2 = [
                'received' => $qty_received,
                'issuance_detail_project' => $_POST['opt_iss_project']
            ];
            $this->as->update_isd($info2, $isd_id[0]);

            // if($_POST['issuance_button'] == 1) {
            //     $this->as->update_item_qty($qty_received, $item_id[0]);
            // }
            $i++;
        }
        
        redirect('Warehouse/admin_issuance', 'refresh');
    }
