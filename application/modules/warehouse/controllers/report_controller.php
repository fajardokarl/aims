...
class report_controller extends CI_Controller
{
	...
	public function report_po() {
        ...
        $poDetails = $this->report->retrieve_po_details($this->input->get('poid'));
        $contactDetails = $this->report->retrieve_po_contact_details($this->input->get('poid'));
		$rrDetails = $this->report->retrieve_po_rr_details($this->input->get('poid'));
        $items = $this->report->retrieve_po_item_availability($this->input->get('poid'));
        $itemAvailability = "";
        $i = 0;
        while($i < count($items)) {
        ...
        $this->data['itemAvailability'] = $itemAvailability;
        $this->data['poDetails'] = $poDetails;
        $this->data['contactDetails'] = $contactDetails;
   ...
   public function generateSalesReport()
	...
	public function poDetailsReport(){
		...
		$this->data['page_title'] = 'Purchase Order Reports';
	...
	public function po_confirm() {
		...
		echo '</pre>';*/
        $info1 = [
        	// 'po_remark' => $_POST['po_status_remark'],
            'po_status' => $_POST['status_clicked'],
            'po_date_received' => $_POST['date_received']
        ];
		$this->report->confirm_po($info1, $po_id);
		...
		while($i < $_POST['inc']) {
			$pod_id = explode('-pod_id', $names[$i]);
			echo $pod_id[0];
			$info2 = [
	        	'po_uom_id' => $_POST[$pod_id[0] . '-po_uom'],
	            'po_received' => $_POST[$pod_id[0] . '-pod_item_qty_received']
        	];
			$this->report->update_pod($info2, $pod_id[0]);
			$i++;
		}
		// redirect('Warehouse/report_controller/report_po?poid=' . $po_id, 'refresh');
	}

    public function generateRR() {
        $parameters = array();
        parse_str($_POST['serializeData'], $parameters);

        $po_id = (int) $parameters['po_id'];
        $po_mod = $parameters['po_mod'];
        // $dr_no = (int) $parameters['dr_no'];
        // $invoice_no = (int) $parameters['invoice_no'];
        $rr_id = $this->report->getRR($po_id);

        // if(!$rr_id) {

            $info = array( 'po_mod' => $po_mod );
            $this->report->updatePOMod($po_id, $info);
            ...
            $data = array(
                ...
                'withholding_tax' => 0,
                'net_amount_due' => 0,
                'delivery_receipt_number' => $parameters['dr_no'],
                'invoice_number' => $parameters['invoice_no'],
                'requisitioned_by_id' => 0,
                'received_by_id' => 0,
            );
            $this->report->generateReceivingReport($data);

            $i = 0;
            $names1 = array(); // pod_id
            $names2 = array(); // item_id
            $names3 = array(); // item_qty
            $warehouse_status = 'served';
            ...
            foreach ($parameters as $name => $val) {
                ...
                } elseif(strpos($name, '-item_qty')) {
                    array_push($names3, $name);
                }
            }

            while($i < $parameters['inc']) {
                $pod_id = explode('-pod_id', $names1[$i]);
                $item_id = explode('-item_id', $names2[$i]);
                // $item_qty = explode('-item_qty', $names3[$i]);
                $item_qty = $parameters[$pod_id[0] . '-item_qty'];

                $qty_received = $parameters[$pod_id[0] . '-pod_item_qty_received'];

                $info2 = [
                    'po_uom_id' => $parameters[$pod_id[0] . '-po_uom'],
                    'po_received' => $qty_received
                ];
                $this->report->update_pod($info2, $pod_id[0]);

                if($item_qty != $qty_received) {
            ...
            $info1 = [
                'po_status' => $warehouse_status
            ];
            $this->report->confirm_po($info1, $po_id);
        // }
        echo $dr_no . " " . $invoice_no;
        // redirect('Warehouse/report_controller/report_po?poid=' . $po_id, 'refresh');s
    }
    ...
    public function cancelPO() {
        ...
        $info = array( 'po_status' => 'cancelled' );

        $this->report->cancel_po($info, $po_id);
    }

	public function pdfPO() {
        $id = $this->input->get('po_id');
        $poDetails = $this->report->retrieve_po_details($id);
        ...
        $head = $this->report->headPO($id);
        $details = $this->report->detailPO($id);
	...
	/*public function update_remark() {
		$po_id = $this->input->post('po_id');
        $info = [
            'po_remark' => $this->input->post('po_status_remark'),
            'po_status' => 'INCOMPLETE'
        ];
		$this->report->confirm_po($info, $po_id);
		redirect('Warehouse/report_controller/report_po?poid=' . $po_id, 'refresh');
	}*/
