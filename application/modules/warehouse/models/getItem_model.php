...
class getItem_model extends CI_Model
{
    ...
    function get_items()
    {
        ...
        $this->db->join('item_inventory ii', 'ii.item_id = i.item_id', 'inner');
        $this->db->join('warehouse w', 'w.warehouse_id = ii.warehouse_id', 'inner');
        $this->db->where('i.status_id', 1);
        $query = $this->db->get();
        return $query->result_array();
    }
    ...
    {
        $this->db->select('warehouse_id, warehouse_description');
