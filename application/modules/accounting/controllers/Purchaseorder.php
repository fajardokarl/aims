...
class Purchaseorder extends CI_Controller
{
	public function __construct() {
		parent::__construct();
		$this->load->model('Purchaseorder_model', 'as');		
		$this->load->helper(array('form', 'url'));
		$this->data['navigation'] = 'accounting/navigation';
		$this->data['customjs'] = 'accounting/purchaseorder_js';
		$this->data['verifies'] = $this->as->get_verifies();
	}

	public function index() {
		$this->data['content'] = 'approved_pos';
		$this->data['page_title'] = 'Approval for Issuance';
		$this->data['PO_details'] = $this->as->retrieve_po_report_details();
		$this->load->view('default/index', $this->data);
	}

	public function purchaseorder() {
		$this->data['content'] = 'purchaseorder';
        ...
        $poDetails = $this->as->retrieve_po_details($this->input->get('po_id'));
        $contactDetails = $this->as->retrieve_po_contact_details($this->input->get('po_id'));
		$items = $this->as->retrieve_po_item_availability($this->input->get('po_id'));
        $itemAvailability = "";
        $i = 0;
        while($i < count($items)) {
	...
	public function po_confirm() {
        ...
        $info1 = [
            ...
            'po_accounting_remark' => $_POST['po_accounting_status_remark'],
            'po_accounting_status' => $_POST['status_clicked']
        ];
        $this->as->confirm_po($info1, $po_id);

        while($ii < $_POST['inc']) { // updates the quantity of the item/s
            $pod_id = explode('-pod_id', $names1[$ii]);
            $item_id = explode('-item_id', $names2[$ii]);
            ...
            // unable to save data in database when multiple loops are executed
            /*while($i < $_POST['inc']) {
                $item_id = explode('-item_id', $names2[$i]);
                $qty = $this->as->get_item_qty($item_id[0]);
                array_push($item, $qty->quantity);
                $i++;
            }*/

            /*$final_quantity = $item[$ii] - ($_POST[$pod_id[0] . '-pod_item_qty_received']);
            $info3 = [
        ...
        redirect('Accounting/Purchaseorder/purchaseorder?po_id=' . $po_id, 'refresh');
	}
